...
for (const user of users) {
  for (const favs in user.favoritesSounds) {
    const soundName = user.favoritesSounds[favs]
    counter[favs] = (counter[favs] || 0) + 1
  }
}
for (const favs in counter) {
  console.log(` ${favs} ha sido gregada ${counter[favs]} veces como favorito`)
}
